...
End-to-end CRUD tests for departments, tax codes, vendors, and items. Uses in-memory stores so creates, edits, and deletes can be verified in a single test run.

**Mock data:** In-memory CRUD stores (departments, tax codes, vendors, inventory items), 1 pre-seeded POS product

---

## 1. Creates department, tax code, vendor, saves item, then verifies on POS screen
...
| 7 | Fill vendor "Premium Wines Inc", contact, phone, email, click Add | "Vendor created" toast; vendor in table |
...
| 3 | Switch to Vendors tab, click Add (empty) | "Vendor name is required" |
...
## 6. Edits and deletes a vendor

| # | Step | Assertion |
|---|------|-----------|
| 1 | Open inventory, Vendors tab | -- |
| 2 | Create vendor "ABC Dist" | "Vendor created"; "ABC Dist" in table |
| 3 | Click the "ABC Dist" row | Edit panel appears |
| 4 | Change name to "XYZ Distributors", click Save | "Vendor saved"; "XYZ Distributors" in table |
| 5 | Click Delete, confirm | "Vendor deleted"; "No vendors yet" |
