...
import { app, shell, BrowserWindow, ipcMain, dialog, crashReporter } from 'electron'
import { initLogger, scoped } from './services/logger'
...
function formatStartupError(error: unknown): string {
  ...
  const isNativeAbiMismatch =
    message.includes('better_sqlite3.node') && message.includes('NODE_MODULE_VERSION')

  if (!isNativeAbiMismatch) return message

  return [
    message,
    '',
    'Detected native module ABI mismatch for better-sqlite3.',
    'Rebuild it for Electron and retry:',
...
process.on('uncaughtException', (error) => {
  log.error('Uncaught exception:', formatStartupError(error))
})
import {
  getPurchaseOrders,
...
import { initializeTelemetry, trackTelemetryEvent, shutdownTelemetry } from './services/telemetry'
...
function createWindow(): void {
  ...
  mainWindow.on('ready-to-show', () => {
    trackTelemetryEvent({
      type: 'performance',
      name: 'app_startup_ready_to_show',
      payload: { duration_ms: Date.now() - appBootStartedAt },
      sampleRate: 1
    })
    mainWindow.show()
  })
...
// Electron crash reporter — writes native crashes to ${userData}/Crashes.
// Kept local (no remote upload) per monitoring-observability.md security constraints.
crashReporter.start({ uploadToServer: false, submitURL: '' })

// Wrap ipcMain.handle to time every IPC call and log errors with channel context.
// Replaces the scattered try/catch-and-rethrow boilerplate that each handler used.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type IpcHandler = (event: Electron.IpcMainInvokeEvent, ...args: any[]) => any

const ipcLog = scoped('ipc')
...
function handle(channel: string, fn: IpcHandler): void {
  ipcMain.handle(channel, async (event, ...args) => {
    ...
    try {
      ...
      ipcLog.debug(`${channel} ok duration_ms=${Date.now() - startedAt}`)
      return result
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err)
      ipcLog.error(`${channel} failed duration_ms=${Date.now() - startedAt} error=${message}`)
      throw err
    }
  })
}
...
app
  ...
  .then(() => {
    ...
    log.info(
      `App starting — version=${app.getVersion()} electron=${process.versions.electron} chrome=${process.versions.chrome} platform=${process.platform} userData=${userData}`
    )
    void initializeTelemetry(userData)
      .then(() => {
        trackTelemetryEvent({
          type: 'system',
          name: 'app_startup_begin',
          payload: { user_data_path: userData },
          sampleRate: 1
        })
      })
      .catch((err) => {
    ...
    handle('telemetry:track', async (_, event: TelemetryEventInput) => {
      trackTelemetryEvent(event)
    })

    handle('products:active-special-pricing', async () => {
      try {
    ...
    handle('products:search', async (_, query: string, filters?: SearchProductFilters) => {
      try {
    ...
    handle('inventory:products:list', async () => {
      try {
    ...
    handle('inventory:products:search', async (_, query: string) => {
      try {
    ...
    handle('inventory:products:detail', async (_, itemNumber: number) => {
      try {
    ...
    handle('inventory:products:save', async (_, payload) => {
      try {
    ...
    handle('inventory:products:delete', async (_, itemNumber: number) => {
      try {
    ...
    handle('inventory:departments:list', async () => {
      try {
    ...
    handle('inventory:item-types:list', async () => {
      try {
        return getInventoryItemTypes()
      } catch (err) {
        throw new Error(err instanceof Error ? err.message : 'Failed to list item types')
    ...
    handle('inventory:tax-codes:list', async () => {
      try {
    ...
    handle('item-types:create', async (_, input) => {
      try {
    ...
    handle('item-types:update', async (_, input) => {
      try {
    ...
    handle('item-types:delete', async (_, id: number) => {
      try {
    ...
    handle('departments:create', async (_, input) => {
      try {
        return createItemType(input)
      } catch (err) {
        throw new Error(err instanceof Error ? err.message : 'Failed to create item type')
    ...
    handle('departments:update', async (_, input) => {
      try {
        return updateItemType(input)
      } catch (err) {
        throw new Error(err instanceof Error ? err.message : 'Failed to update item type')
    ...
    handle('departments:delete', async (_, id: number) => {
      try {
        return deleteItemType(id)
      } catch (err) {
        throw new Error(err instanceof Error ? err.message : 'Failed to delete item type')
    ...
    handle('tax-codes:create', async (_, input) => {
      try {
    ...
    handle('tax-codes:update', async (_, input) => {
      try {
    ...
    handle('tax-codes:delete', async (_, id: number) => {
      try {
    ...
    // Distributor CRUD
    handle('distributors:list', async () => {
      try {
    ...
    handle('distributors:create', async (_, input) => {
      try {
    ...
    handle('distributors:update', async (_, input) => {
      try {
    ...
    handle('distributors:delete', async (_, distributorNumber: number) => {
      try {
    ...
    // Sales Rep CRUD
    handle('sales-reps:list-by-distributor', async (_, distributorNumber: number) => {
      try {
    ...
    handle('sales-reps:create', async (_, input) => {
      try {
    ...
    handle('sales-reps:update', async (_, input) => {
      try {
    ...
    handle('sales-reps:delete', async (_, salesRepId: number) => {
      try {
    ...
    // Merchant Config
    handle('merchant:get-config', async () => {
      try {
    ...
    handle('merchant:deactivate', async () => {
      try {
    ...
    // Catalog
    handle('catalog:distributors', async () => {
      try {
    ...
    handle('catalog:import', async (_, distributorIds: number[]) => {
      try {
    ...
    handle('cashiers:create', async (_, input) => {
      try {
    ...
    handle('cashiers:validate-pin', async (_, pin: string) => {
      try {
    ...
    handle('cashiers:update', async (_, input) => {
      try {
    ...
    handle('cashiers:delete', async (_, id: number) => {
      try {
    ...
    // Transactions
    handle('transactions:save', async (_, input: SaveTransactionInput) => {
      try {
    ...
    handle('transactions:recent', async (_, limit?: number) => {
      try {
    ...
    handle('transactions:get-by-number', async (_, txnNumber: string) => {
      try {
    ...
    handle('transactions:save-refund', async (_, input: SaveRefundInput) => {
      try {
    ...
    handle('transactions:list', async (_, filter: TransactionListFilter) => {
      try {
    ...
    // Held Transactions
    handle('held-transactions:save', async (_, input: SaveHeldTransactionInput) => {
      try {
    ...
    handle('held-transactions:list', async () => {
      try {
    ...
    handle('held-transactions:delete', async (_, id: number) => {
      try {
    ...
    handle('held-transactions:clear-all', async () => {
      try {
        clearAllHeldTransactions()
      } catch (err) {
    ...
    // Sessions
    handle('sessions:get-active', async () => {
      try {
    ...
    handle('sessions:create', async (_, input: CreateSessionInput) => {
      try {
    ...
    handle('sessions:close', async (_, input: CloseSessionInput) => {
      try {
    ...
    handle('sessions:list', async (_, limit?: number, offset?: number) => {
      try {
    ...
    handle('sessions:report', async (_, sessionId: number) => {
      try {
    ...
    handle('sessions:print-report', async (_, input: PrintClockOutReportInput) => {
      try {
    ...
    // Finix Payments
    handle('finix:devices:list', async () => {
      try {
    ...
    handle('finix:device:create', async (_, input: FinixCreateDeviceInput) => {
      try {
        const config = getMerchantConfig()
    ...
    handle('finix:charge:card', async (_, input: FinixCardInput) => {
      try {
        const config = getMerchantConfig()
    ...
    handle('finix:charge:terminal', async (_, input: FinixTerminalChargeInput) => {
      try {
        const config = getMerchantConfig()
        if (!config) throw new Error('Merchant not activated — cannot process payments')
    ...
    handle('finix:void:authorization', async (_, authorizationId: string) => {
      try {
        const config = getMerchantConfig()
    ...
    handle('finix:refund:transfer', async (_, transferId: string, amountCents: number) => {
      try {
        const config = getMerchantConfig()
        if (!config) throw new Error('Merchant not activated')
    ...
    handle('finix:provision-merchant', async (_, input: BusinessInfoInput) => {
      try {
    ...
    // Cash Drawer
    handle('peripheral:get-drawer-config', () => {
      return getCashDrawerConfig()
    })

    handle('peripheral:save-drawer-config', (_, config: CashDrawerConfig) => {
      saveCashDrawerConfig(config)
    })

    handle('peripheral:open-cash-drawer', async () => {
    ...
    handle('peripheral:print-receipt', async (_, input: PrintReceiptInput) => {
      try {
    ...
    handle('peripheral:get-receipt-config', () => {
      return getReceiptConfig()
    })

    handle('peripheral:save-receipt-config', (_, config: ReceiptConfig) => {
      saveReceiptConfig(config)
    })

    handle('peripheral:get-receipt-printer-config', () => {
      return getReceiptPrinterConfig()
    })

    handle('peripheral:save-receipt-printer-config', (_, config: ReceiptPrinterConfig) => {
      saveReceiptPrinterConfig(config)
    })

    handle('peripheral:list-receipt-printers', async () => {
      return await listReceiptPrinters()
    })

    handle('peripheral:get-printer-status', async (_, printerName?: string) => {
    ...
    handle('inventory:apply-tax-to-all', async (_, taxCodeId: number) => {
      try {
    ...
    handle('sync:get-initial-status', () => {
      return getInitialSyncStatus()
    })

    handle('sync:retry-initial-sync', async () => {
      try {
    ...
    handle('sync:get-device-config', async () => {
      return getDeviceConfig()
    })

    // ── Transaction history IPC ──

    handle('history:get-stats', async () => {
      return getLocalTransactionHistoryStats()
    })

    handle('history:get-backfill-status', async () => {
      return getBackfillStatus()
    })

    handle('history:trigger-backfill', async (_, days: number) => {
      const merchantCloudId = await getMerchantCloudId()
      if (!merchantCloudId) throw new Error('Not authenticated')
      const client = getSupabaseClient()
      ...
      if (numericDays > MAX_SAFE_BACKFILL_DAYS) {
        log.warn(
          `manual backfill requested beyond safe window — days=${numericDays} (max safe=${MAX_SAFE_BACKFILL_DAYS})`
        )
      }

      // Fire and forget — caller polls history:get-backfill-status or subscribes
      // to history:backfill-status-changed events to observe progress.
      void backfillTransactions(client, merchantCloudId, numericDays).catch((err) => {
        log.error(`manual backfill failed: ${err instanceof Error ? err.message : String(err)}`)
      })

      return { started: true, days: numericDays }
    })

    // ── Reports IPC ──

    handle('reports:sales-summary', async (_, range: ReportDateRange) => {
      try {
    ...
    handle('reports:category-sales', async (_, range: ReportDateRange) => {
      try {
    ...
    handle('reports:tax-summary', async (_, range: ReportDateRange) => {
      try {
    ...
    handle('reports:comparison', async (_, rangeA: ReportDateRange, rangeB: ReportDateRange) => {
      try {
    ...
    handle('reports:cashier-sales', async (_, range: ReportDateRange) => {
      try {
    ...
    handle('reports:hourly-sales', async (_, range: ReportDateRange) => {
      try {
    ...
    handle('reports:export', async (_, request: ReportExportRequest) => {
      try {
    ...
    // Push transaction backfill status changes to renderer
    onBackfillStatusChange((status) => {
      const win = BrowserWindow.getAllWindows()[0]
  ...
  .catch((error) => {
    ...
    console.error('[startup] Fatal initialization error:', message)
    try {
      if (app.isReady()) {
        dialog.showErrorBox('High Spirits POS failed to start', message)
      }
    } catch {
      // ignore UI reporting failures during early startup
    }
    app.exit(1)
  })
...
handle('auth:set-session', async (_, accessToken: string, refreshToken: string) => {
  try {
...
handle('auth:consume-pending-deep-link', () => {
...
handle('registers:rename', async (_, id: string, newName: string) => {
  try {
    const supabase = getSupabaseClient()
...
handle('registers:delete', async (_, id: string) => {
  try {
    const supabase = getSupabaseClient()
    if (!supabase) throw new Error('Not connected to cloud')
...
handle('inventory:reorder-distributors', async () => {
  try {
    return getDistributorsWithReorderable()
  } catch (err) {
    throw new Error(err instanceof Error ? err.message : 'Failed to get reorder distributors')
  }
})

handle('inventory:set-discontinued', async (_, id: number, discontinued: boolean) => {
  try {
    setProductDiscontinued(id, discontinued)
  } catch (err) {
    throw new Error(err instanceof Error ? err.message : 'Failed to update discontinued flag')
  }
})

// Unpriced products (active items with $0 price)
handle('inventory:unpriced-products', async () => {
  try {
...
handle('inventory:find-by-sku', async (_, sku: string) => {
  try {
...
handle('inventory:distinct-sizes', async () => {
  try {
...
handle('finix:merchant-status', async () => {
  try {
    const config = getMerchantConfig()
...
handle('purchase-orders:list', async () => {
  try {
...
handle('purchase-orders:detail', async (_, poId: number) => {
  try {
...
handle('purchase-orders:create', async (_, input: CreatePurchaseOrderInput) => {
  try {
...
handle('purchase-orders:update', async (_, input: UpdatePurchaseOrderInput) => {
  try {
...
handle('purchase-orders:receive-item', async (_, input: ReceivePurchaseOrderItemInput) => {
  try {
...
handle('purchase-orders:remove-item', async (_, poId: number, itemId: number) => {
  try {
...
handle('purchase-orders:delete', async (_, poId: number) => {
  try {
...
handle('zoom:get', (event) => event.sender.getZoomFactor())
handle('zoom:set', (event, factor: number) => {
